...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--skip-stripe', action='store_true',
                             help='Skip Stripe sync — faster for local dev when you only need feed/content data.')
    @transaction.atomic
    def handle(self, *args, **options):
        ...
        self._create_tiers(creators, skip_stripe=options['skip_stripe'])
    ...
    def _create_tiers(self, creators, skip_stripe=False):
        for creator in creators:
            ...
            for level, name in enumerate(TIER_NAMES, start=1):
                ...
                if not skip_stripe:
                    ...
                    self.stdout.write(f'  Synced to Stripe: {tier}')
