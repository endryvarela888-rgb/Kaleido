...
def _send_activation_email(request, user):
    """
    Builds a link into the React app, not a Django URL — the SPA is what
    owns the /activate/:uid/:token route and exchanges the token for a
    JWT session via the API. `request` is kept as a parameter for call-site
    compatibility even though it's no longer used to build the link.
    """
...
def _send_password_reset_email(user):
    """Same idea as _send_activation_email: the link points at the React
    app's /reset-password/:uid/:token route. Uses Django's own
    default_token_generator (the same one PasswordResetForm relies on
    internally), so a token here is single-use and expires exactly like
    the built-in Django flow."""
    uid = urlsafe_base64_encode(force_bytes(user.pk))
    token = default_token_generator.make_token(user)
    reset_link = f'{settings.FRONTEND_URL}/reset-password/{uid}/{token}'
    message = render_to_string('users/api_password_reset_email.html', {
        'user': user,
        'reset_link': reset_link,
    })
    send_mail(
        subject='Reset your Kaleido password',
        message=message,
        from_email=settings.DEFAULT_FROM_EMAIL,
        recipient_list=[user.email],
    )
